...
FlipControlPacket FlipControlDataBuffer[DATA_BUFFER_SIZE];
...
void sendFlipControlData(int i)
{
  tx_estring_value.clear();
  sprintf(buffer, "%ld", FlipControlDataBuffer[i].time);
  tx_estring_value.append(buffer);
  tx_estring_value.append("|");
  tx_estring_value.append(FlipControlDataBuffer[i].speed);
  tx_estring_value.append("|");
  tx_estring_value.append(FlipControlDataBuffer[i].tof_distance);
  tx_estring_value.append("|");
  tx_estring_value.append(FlipControlDataBuffer[i].kalman_distance);
  tx_characteristic_string.writeValue(tx_estring_value.c_str());

  Serial.println("Sending Flip Control Data");
}
/*
...
int flip_speed = 0;
int record_data = 0;
int flip_distance_threshold = 0;
int flip_tof_timing_budget = 0;

int flip_distance = 0;
bool flip_data_ready = false;
...
FlipState flipState = FLIP_FORWARD;
// =========================== END: FLIP ==================================
enum CommandTypes
{
  PING,
...
void handle_command()
{
  ...
  case SEND_PID_DATA:
    ...
    break;
  case FLIP:
    success = robot_cmd.get_next_value(do_flip);
    if (!success)
      return;
    success = robot_cmd.get_next_value(flip_speed);
    if (!success)
      return;
    success = robot_cmd.get_next_value(record_data);
    if (!success)
      return;
    success = robot_cmd.get_next_value(flip_distance_threshold);
    if (!success)
      return;
    success = robot_cmd.get_next_value(flip_tof_timing_budget);
    if (!success)
      return;
    success = robot_cmd.get_next_value(back_delay);
    if (!success)
      return;

    distanceSensor1.setTimingBudgetInMs(flip_tof_timing_budget);
    break;
  case SEND_FLIP_DATA:
    for (int i = 0; i < dataIndex; i++)
    {
      sendFlipControlData(i);
    }
    dataIndex = 0;
    break;
  default:
...
void record_flip_control_data(int i, int time, int speed, int distance, float kalman_distance)
{
  FlipControlDataBuffer[i].time = time;
  FlipControlDataBuffer[i].speed = speed;
  FlipControlDataBuffer[i].tof_distance = distance;
  FlipControlDataBuffer[i].kalman_distance = kalman_distance;
}
  ...
  if (central)
  {
    ...
    while (central.connected())
    {
      ...
      if (do_flip)
      {
        ...
        if (nonBlockReadTOF1(&distance1) == 0)
        {
          KF(true, speed / flip_speed, -distance1);
        }
        else
        {
          KF(false, speed / flip_speed, -distance1);
        }

        flip_distance = x(0, 0);
        // TODO: Some logic to get the distance from the wall (KF)
        if (flipState == FLIP_FORWARD)
        {

          moveForward(flip_speed);
          speed = flip_speed;
          if (flip_distance < flip_distance_threshold)
          {
            flipState = FLIP_BACKWARD;
          }
        }
        else if (flipState == FLIP_BACKWARD)
        {
          moveBackward(flip_speed);
          speed = -flip_speed;
          delay(back_delay); // FIXME: something more robust? maybe pass in time as a command argument
          flipState = FLIP_STOP;
        }
        else if (flipState == FLIP_STOP)
        {
          stop();
        ...
        if (record_data)
        {
          record_flip_control_data(dataIndex, current_time, speed, distance1, x(0, 0));
          dataIndex++;
        }
      }
      else
